...
@Component({
  ...
  template: `
    @switch (level()) {
      @case (1) { <h1 [id]="headingId()" [class]="classes()">{{ text() }}</h1> }
      @case (2) { <h2 [id]="headingId()" [class]="classes()">{{ text() }}</h2> }
      @case (3) { <h3 [id]="headingId()" [class]="classes()">{{ text() }}</h3> }
      @default { <h4 [id]="headingId()" [class]="classes()">{{ text() }}</h4> }
    }
  `,
})
